# GUI Fetch Page Code Review Fixes Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 修复 code review 发现的 8 个问题：竞态、连接泄漏、错误处理、代码卫生。

**Architecture:** 所有修改集中在 `gui/pages/fetch.py` 一个文件。提取日期计算为辅助函数，修复 is_running 守卫，用 try/finally 管理 DB 生命周期，使用 app_state.output_dir，移除未使用导入。

**Tech Stack:** NiceGUI, Python sqlite3

---

### Task 1: 修复 gui/pages/fetch.py 的 8 个 code review 问题

**Files:**
- Modify: `gui/pages/fetch.py`

- [ ] **Step 1: 完整重写 gui/pages/fetch.py**

将整个文件替换为以下修复版本（所有 8 个问题一次性修复）：
...
| # | 问题 | 修复方式 | 位置 |
|---|------|---------|------|
| 1 | on_run_single 缺少 is_running 守卫 | 添加 `if is_running["value"]: return` | on_run_single 入口 |
| 2 | on_run_basket 缺少 is_running 守卫 | 添加 `if is_running["value"]: return` | on_run_basket 入口 |
| 3 | 页面重导航连接泄漏 | 拆分 `_build_basket_form(db)`，用 try/finally 关闭 db | _create_basket_tab |
| 4 | 异常路径连接泄漏 | db 创建和 form 构建都包在 try/finally 中 | _create_basket_tab |
| 5 | broad except 吞掉错误 | 添加 `logging.getLogger().warning()` 记录日志 | _update_preview |
| 6 | 未使用导入 Path 和 app_state | 移除 `from pathlib import Path`，app_state 现在被使用 | imports |
| 7 | 硬编码 'data/' | 改为 `app_state.output_dir` | 两处 output_input |
| 8 | 日期计算重复 | 提取 `_default_date_range()` 函数 | 两处调用 |
...
git commit -m "修复(gui): 修复 fetch 页面 8 个 code review 问题

- 添加 is_running 守卫防止双击竞态启动重复子进程
- 用 try/finally 管理 IndustryDB 生命周期，修复连接泄漏
- 拆分 _build_basket_form 便于 db 生命周期管理
- 移除未使用的 Path 和 app_state 导入
- 输出路径改用 app_state.output_dir 保持一致性
- 提取 _default_date_range() 消除日期计算重复
- _update_preview 异常添加 logging 记录便于调试"
```
